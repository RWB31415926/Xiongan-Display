...
window.barLine_short = function() {
        ...
        // 生成“浮动一下”的目标数据（只改一个索引）
        function jitterData(idx) {
            const upOrDown = () => (Math.random() < 0.5 ? -1 : 1);

            // 柱子用相对百分比的小幅波动；给一个最小像素量级，避免小值看不出动静
            const b1 = base.bar1[idx];
            const b2 = base.bar2[idx];
            ...
            // 折线用绝对量级的微小波动（km/h）
            const l0 = base.line[idx];
            const dL = (0.2 + Math.random() * 1) * upOrDown(); // ~0.2~1.0 km/h
            ...
            newBar1[idx] = clamp(b1 + d1, 0, 120000);
            newBar2[idx] = clamp(b2 + d2, 0, 50000);
            newLine[idx] = +clamp(l0 + dL, 30, 45).toFixed(2);
        ...
        myChart.__jitterTimer = setInterval(function() {
            const len = base.bar1.length;
            // 顺序依次取索引
            lastIdx = (lastIdx + 1) % len;
            let idx = lastIdx;

            const { newBar1, newBar2, newLine } = jitterData(idx);
